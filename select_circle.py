...
exit_button = pygame_gui.elements.UIButton(relative_rect=pygame.Rect((window_surface.get_width() - 50 - 176, window_surface.get_height() - 655),
                                                                     (176, 63)), text='Выйти', manager=manager)
start_button = pygame_gui.elements.UIButton(relative_rect=pygame.Rect((window_surface.get_width() - 50 - 176, window_surface.get_height() - 578),
    ...
    while is_running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                choosen_sprite = None # координаты выбранной ячейки равны None
                colors = [(170, 200, 230)] * 15
                for (i, x, y) in holes:
                    if math.hypot(x1 - (x + 35), y1 - (y + 35)) <= 30:
                        colors[i] = (130, 130, 130)
                        choosen_sprite = (x, y) # координаты выбранной ячейки

                        # если сообщение о просьбе выбрать ячейку уже есть на моменте нажатия ячейки - сообщение исчезает
                        if false_mess != '':
                            false_mess = ''
                            please = ''

                clicks_pos.append(choosen_sprite) # в список добавляется либо координаты ячейки, либо None
